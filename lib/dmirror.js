(function (exports) {
    ...
    Raid.implements({
        ...
        event_emitter: function (event, name) {
            loggin.log("info", ["event_emitter", event, name]);
        ...
        event_emitter_rename: function (name) {
            ...
            default:
                loggin.log("error", "multiple actions! should all be create! btw. Do it? ", action_done);
                ...
                if (!action_done) {
                    ...
                    while (i--) {
                        if (diff[i].state === "missing") {
                            if (diff[i].isDirectory()) {
                                directories_to_remove.push(diff[i].rel_filename);
                                this.unwatch_directory(diff[i]);
                            } else {
                                files_to_remove.push(diff[i].rel_filename);
                            }
                        }
                    }
                    if (files_to_remove.length || directories_to_remove.length) {
                        this.__action_queue.push(function (work) {
                            this.__target.rmlist(files_to_remove, function () {
                                this.__target.rmdirlist(directories_to_remove, function () {
                                    work.done();
                                }.bind(this));
                            }.bind(this));
                        }.bind(this)).fire();
                    ...
                    loggin.log("info", "put files = " + files.length + " directories = " + directories.length);
